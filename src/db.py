...
class ClassesDB:
    ...
    def init_db(self):
        with open(SCHEMA_PATH, encoding="utf-8") as scheme:
            self.conn.executescript(scheme.read())
            self.commit_or_rollback()

    def student_exists(self, discord_id: int) -> bool:
        cur = self.conn.cursor()
        cur.execute(
            "SELECT discord_id FROM students WHERE discord_id = ?;",
            (discord_id,),
        )
    ...
    def add_student(self, discord_id: int):
        self.conn.execute(
            "INSERT INTO students (discord_id) VALUES (?);", (discord_id,)
        )
        self.commit_or_rollback()
    ...
    def reset_enrolls(self):
        ...
        self.conn.execute(
            ...
            WHERE
                first_choice IS NOT NULL AND
                second_choice IS NOT NULL;""",
            (self.max_prio,),
        )
    ...
    def get_users_school(self, dc_id: int) -> School | None:
        ...
        cur.execute(
            "SELECT school FROM students WHERE discord_id = ?;", (dc_id,)
        )

        # I think these Index None but it might be okay,
        # bc there's no situation when we might fetch a user not present in the db yet.
        return cur.fetchone()[0]

    def get_users_message(self, dc_id: int) -> int | None:
        cur = self.conn.cursor()
        cur.execute(
            "SELECT invt_msg_id FROM students WHERE discord_id = ?;", (dc_id,)
        )
    ...
    def csv_export(self):
        ...
        while row := cur.fetchone():
            dc_id, fc, sc, prio, roll, sch, stat = row
            writer.writerow(
                (dc_id, ds(fc), ds(sc), prio, roll, ds(sch), dst(stat))
            )
    ...
    def set_priority(self, discord_id: int, prio: int):
        ...
        rows = self.conn.execute(
            "UPDATE students SET priority = ? WHERE discord_id = ?",
            (prio, discord_id),
        ).rowcount
